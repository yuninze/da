...
with t0:
    ...
    st.markdown("국채 일드, 기업채 일드 스프레드 등 적시성과 대표성이 높은 상품(호가)와 지표를 표시한다.")
    cols=["by","iy","cb","ys","fr"]
    ...
    st.markdown("인플레이션 반영 10년 국채 일드는 국채 10년물 수익률을 CPI, CPCE로 deflating한 것으로 명목 국채 일드가 된다.")
    ...
    st.markdown("U.S.10Y, 기업채 스프레드, H.S.I., W.T.I., 10년 인플레이션 기대율의 1차 변화량의 Pearson 상관계수이다. 계열 관계를 톺아보는 단초로 된다.")
    freq="2d"
...
with t3:
    ...
    st.markdown("단선형회귀를 수행한다. 이 대시보드에서 다루는 데이터는 비정상이다. 여기서는 아무런 변환을 수행하지 않으므로 의사회귀가 발생한다. 금융상품 가격은 시중 유동성 총량에 비례하므로 원자재와 지수 선물 가격은 양의 상관관계를 가지는 듯하다. 안전상품인 국채 일드와 위험상품인 항셍 가격 사이에는 음의 상관관계가 있는 듯하다. 그러나 사실 각 계열 사이의 선형계수는, 특히 sampling rate가 높을 수록 낮아진다. 대표적으로, 국채 일드가 상승하면 귀금속 가격이 하락해야 하지만 그렇지 않다. 목적지는 정해져 있는 random walking에서 각 계열 사이의 선형관계는 옅어진다. 이에, 10YIE, CPI 등 유력한 지표의 범위를 바탕으로 grouping해 조건하 상관성을 톺아볼 수 있을 것이다.")
    q0,q1=st.columns(2)
    ...
    if not x0_==y0_:
        ...
        residue_dev=np.std(residue)*.5
        ax[1].scatter(y1_guess,y1_guess-y1_answer,
            color="tomato",alpha=.3)
        ax[1].hlines(
            y=0,
            xmin=min(y1_guess)-residue_dev,
            xmax=max(y1_guess)+residue_dev,
            color="navy",alpha=1)
        # render
        ...
        st.markdown("Select x (enobs), y (exobs)")
    # row 1
    with st.container():
        st.subheader("Tests")
